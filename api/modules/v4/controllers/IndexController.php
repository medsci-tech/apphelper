...
class IndexController extends \api\common\controllers\Controller
{
    ...
    public function actionIndex()
    {
        ...
        if(!$data)
        {
            ...
            $exams = Exam::find()->select(['id','name as title','imgurl',"LENGTH(exe_ids) - LENGTH( REPLACE(exe_ids,',','')+1) as total",'publish_time'])->OrderBy($orderBy)->where($where)->asArray()->all(); //所有推荐资源
            if($exams)
            {
    ...
    {
        $result = ['code' => 200,'message'=>'版本升级提醒','data'=>['lastVersion'=>'0.9.4','download'=>'http://mydgt.com/mydisk/DoctorHelper_P1_0.9.4.apk','isUpdate'=>true,'isCompel'=>false]];
        return $result;
    }
