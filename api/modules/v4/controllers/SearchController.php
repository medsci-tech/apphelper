...
class SearchController extends \api\common\controllers\Controller
{
    ...
    public function actionIndex()
    {
        ...
        $exams = Exam::find()->select(['id','name as title','imgurl',"if(type>0,total,LENGTH(exe_ids) - LENGTH(REPLACE(exe_ids,',',''))+1)AS total",'publish_time'])->OrderBy($orderBy)->where($where)->andWhere(['like', 'name', $keyword])->asArray()->all(); //所有推荐资源
        if($exams)
        {
